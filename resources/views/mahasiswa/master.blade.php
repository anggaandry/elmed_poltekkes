...
<body>
                            ...
                            <div class="dlab-side-menu">
                                ...
                                <div class="sidebar-social-link ">
                                    <ul>
                                        ...
                                        <li class="nav-item ">
                                            @if ($auth->lang == 'id')
                                                <a class="btn btn-info m-1 px-3 py-2" href="{{ url('/mahasiswa/auth/lang?id=' . $auth->id . '&lang=en') }}"><img src="{{ asset('images/icon/english.png') }}" height="25" alt="language"></a>
                                            @else
                                                <a class="btn btn-info m-1 px-3 py-2" href="{{ url('/mahasiswa/auth/lang?id=' . $auth->id . '&lang=id') }}"><img src="{{ asset('images/icon/bahasa.png') }}" height="25" alt="language"></a>
                                            @endif
                                        </li>
                                        <li class="nav-item ">
        ...
        ***********************************-->
        <div class="content-body">
            ...
            <div class="container-fluid">
                ...
                <div class="modal fade" id="absence_header">
    ...
    ***********************************-->
    ...
    <script src="{{ asset('template/js/dlabnav-theme-4.js') }}"></script>
    ...
    <script>
        ...
        function check() {
            ...
            var view_ = $('#absence_view_header');
            var modal_ = $('#absence_header');
            $.ajax({
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                },
                url: "{{ url('mahasiswa/absence/check') }}",
                success: function(data) {
                    console.log(data);

                    if (data.result.str) {
                        view_.removeClass('d-none');
                        if (show_abse && !data.result.absence) {
                            modal_.modal('show');
                            show_abse = false;
                        }

                        $('#class_ah').html(data.result.str.class_name);
                        $('#sks_ah').html(data.result.str.sks_name);
                        $('#room_ah').html(data.result.str.room_name);
                        $('#schedule_ah').html(data.result.str.schedule_name);
                        $('#lecturer_ah').html(data.result.str.lecturer);
                        $('#start_id_ah').val(data.result.str.id);
                        $('#schedule_id_ah').val(data.result.str.schedule_id);
                        countDownDatey = new Date(data.result.str.timer).getTime();

                        if (data.result.absence) {
                            fade = false;
                            $('#status_ah').val(data.result.absence.status).change();
                            $('textarea#note_ah').val(data.result.absence.note);
                        } else {
                            fade = true;
                        }
                    } else {
                        view_.addClass('d-none');
                        fade = false;
                        countDownDatey = null;
                    }
                },
        ...
        $(document).on('submit', '#form_absence_header', function(event) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.code == 1) {
                        ...
                        check();
                        $('#absence_header').modal('hide');
                        $('#form_absence_header')[0].reset();
                    } else {
                        show_toast(0, data.message);
                    }
                    $('#load_ah').addClass('d-none');
                },
                error: function(request, status, error) {
                    console.error(request.responseText);
    ...
    <script>
        ...
        var countDownDatey = null;
        var fade = false;
        var boo_c = false;

        // Update the count down every 1 second
        var y = setInterval(function() {
            if (countDownDatey != null) {
                // Get today's date and time
                var nowy = new Date().getTime();

                // Find the distance between now and the count down date
                var distancey = countDownDatey - nowy;
                ...
                var conty = "<span class='badge badge-danger'>";
                if (daysy > 0) {
                    conty += (daysy + " Hari ");
                }
                ...
                document.getElementById("timer_ah").innerHTML = conty +
                    minutesy + " menit " + secondsy + " detik </span> ";

                if (fade) {
                    if (boo_c) {
                        $('#btn33').addClass('bg-danger');
                        $('#btn33').removeClass('bg-warning');
                    } else {
                        $('#btn33').removeClass('bg-danger');
                        $('#btn33').addClass('bg-warning');
                    }
                    boo_c = !boo_c;
                } else {
                    $('#btn33').removeClass('bg-danger');
