...
@section('breadcrumb')
    <div class="row page-titles">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="javascript:void(0)">{{ tr('edit role') }}</a></li>
        </ol>
    </div>
@endsection
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="mb-3 col-md-6">
                                ...
                                <input type="text" class="form-control" name="name" placeholder="Ex. Admin/Inventory/Keuangan/Admin Fakutas A" value="{{ $role_data->name }}" required>
                            </div>
                            <div class="mb-3 col-md-6">
                                <label class="form-label">{{ tr('akses prodi') }}</label>
                                <select class="form-select form-select-lg" name="prodi_id" onchange="change_prodi(this.value)">
                                    <option value="" selected>{{ tr('semua akses') }}</option>
                                    @foreach ($spc_data as $item)
                                        <option value="{{ $item->id }}" @if ($role_data->prodi_id == $item->id) selected @endif>{{ $item->program->name }}
                                                    ...
                                                    <td class="text-center">
                                                        @if ($item->has_view)
                                                            <div class="custom-checkbox checkbox-primary">
                                                                <input type="checkbox" class="form-check-input {{ $item->only_prodi == 0 ? 'only-prodi' : '' }}" @if ($item->access) @if ($item->access->view_access == 1)
                                                                            checked @endif @endif
                                                                name="cbView{{ $item->id }}"
                                                    ...
                                                    <td class="text-center">
                                                        @if ($item->has_add)
                                                            <div class="custom-checkbox checkbox-success">
                                                                <input type="checkbox" class="form-check-input {{ $item->only_prodi == 0 ? 'only-prodi' : '' }}" @if ($item->access) @if ($item->access->add_access == 1)
                                                                    checked @endif @endif
                                                                name="cbAdd{{ $item->id }}">
                                                            </div>
                                                        @endif
                                                    ...
                                                    <td class="text-center">
                                                        @if ($item->has_edit)
                                                            <div class="custom-checkbox checkbox-info">
                                                                <input type="checkbox" class="form-check-input {{ $item->only_prodi == 0 ? 'only-prodi' : '' }}" @if ($item->access) @if ($item->access->edit_access == 1)
                                                                    checked @endif @endif
                                                                name="cbEdit{{ $item->id }}">
                                                            </div>
                                                        @endif
                                                    ...
                                                    <td class="text-center">
                                                        @if ($item->has_delete)
                                                            <div class="custom-checkbox checkbox-danger">
                                                                <input type="checkbox" class="form-check-input {{ $item->only_prodi == 0 ? 'only-prodi' : '' }}" @if ($item->access) @if ($item->access->delete_access == 1)
                                                                    checked @endif @endif
